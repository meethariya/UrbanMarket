...
public class InventoryController {
    ...
    public ResponseEntity<UMResponse> createProduct(@ModelAttribute RequestInventoryDto inventoryDto) throws IOException {
        ResponseInventoryDto responseInventoryDto = inventoryService.createProduct(inventoryDto);
        UMResponse umResponse = UMResponse.builder().data(responseInventoryDto).status(UMResponse.ResponseStatus.SUCCESS).build();
    ...
    public ResponseEntity<UMResponse> getInventories(@PathParam(value = "productId") String productId){
        ...
        UMResponse umResponse = UMResponse.builder().data(responseInventoryDtos).status(UMResponse.ResponseStatus.SUCCESS).build();
        return ResponseEntity.ok(umResponse);
    }

	/**
	 * Update inventory details.<br>
	 * Throws {@link com.urbanmarket.inventoryservice.exception.InventoryNotFoundException}  and
	...
	public ResponseEntity<UMResponse> deleteInventoryById(@RequestParam String id){
        ...
        return ResponseEntity.ok(UMResponse.builder().data("Deleted successfully").status(UMResponse.ResponseStatus.SUCCESS).build());
	}
}